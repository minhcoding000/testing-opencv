...
def region_of_interest(image):
    ...
    polygon = np.array([(0, height), (1100, height),
                        (1100, height-300), (450, 350)])
    ...
    # computing bitwise of two images
    masked_image = cv2.bitwise_and(image, mask)
    return masked_image


image = cv2.imread('minhgray.jpg')
lane_image = np.copy(image)
canny_image = canny(lane_image)
cropped_image = region_of_interest(canny_image)
lines = cv2.HoughLinesP(cropped_image, 2, np.pi/180, 100, np.array([]),
                        minLineLength=40, maxLineGap=5)  # single degree precision
if (lines is not None):
